...
class EventFactoryTest {
  ...
  @InjectMocks private EventFactory eventFactory;
  ...
  void setUp() {
    ...
    testEvent =
        new Event(
            1, 1, "Music Event", "A live music concert", "The Venue", 100, new Date(), new Date());
  }
  ...
  void testSave() throws SQLException {
    ...
    verify(mockConnection)
        .executeUpdate(
            eq(
                "UPDATE Events SET CommunityId = ?, Name = ?, Description = ?, Venue = ?, Capacity = ?, Scheduled = ?, Created = ? WHERE Id = ?"),
            captor.capture(),
            any());
  ...
  void testCreate() throws SQLException {
    ...
    verify(mockConnection)
        .executeUpdate(
            eq(
                "INSERT INTO Events (CommunityId, Name, Description, Venue, Capacity, Scheduled, Created) VALUES (?, ?, ?, ? ,? ,?, ?)"),
            captor.capture(),
            any());
