...
/**
 * Represents the context of the current user. This includes the user model and the current
...
public class UserContext {
  ...
  public Set<Scope> getCurrentScopes(Community community) {
    var communityUser =
        communityUsers.stream().filter(c -> c.getCommunityId() == community.getId()).findFirst();
    return communityUser
        .map(communityUserModel -> ScopeUtils.deconstructScopes(communityUserModel.getPermission()))
